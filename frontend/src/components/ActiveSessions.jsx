      ...
      <div className="card-body">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-2xl font-black">Các Session đang Live</h2>
          </div>

          <div className="flex items-center gap-2">
            <div className="size-2 bg-success rounded-full" />
            <span className="text-sm font-medium text-success">{sessions.length} đang hoạt động</span>
          </div>
        </div>
        ...
        <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
          {isLoading ? (
            ...
            sessions.map((session) => (
              ...
              >
                <div className="flex items-center justify-between gap-4 p-5">
                  ...
                  <div className="flex items-center gap-4 flex-1">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-2">
                        <h3 className="font-bold text-lg truncate">{session.problem || "Phỏng vấn"}</h3>
                        {session.difficulty && (
                          <span
                      ...
                      <div className="flex items-center gap-4 text-sm opacity-80">
                        ...
                        {session.participant && !isUserInSession(session) ? (
                          <span className="badge badge-error badge-sm">ĐÃ ĐẦY</span>
                        ) : (
                          <span className="badge badge-success badge-sm">ĐANG MỞ</span>
                        )}
                      </div>
                    </div>
                  </div>
                  ...
                  {session.participant && !isUserInSession(session) ? (
                    <button className="btn btn-disabled btn-sm">Đã đầy</button>
                  ) : (
                    <Link to={`/session/${session._id}`} className="btn btn-primary btn-sm gap-2">
                      {isUserInSession(session) ? "Vào lại" : "Tham gia"}
            ...
            <div className="text-center py-16">
              ...
              <p className="text-lg font-semibold opacity-70 mb-1">Chưa có Session nào đang hoạt động</p>
              <p className="text-sm opacity-50">Hãy là người đầu tiên tạo Session!</p>
            </div>
          )}
        </div>
      </div>
